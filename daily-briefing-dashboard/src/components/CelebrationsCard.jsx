...
function ConfettiScene() {
  const pieces = useMemo(() => Array.from({ length: 30 }, (_, i) => ({
    ...
    dur:          `${2.5 + pr(i, 5.1) * 2.5}s`,
    width:        `${3 + Math.floor(pr(i, 2.9) * 4)}px`,
    height:       `${5 + Math.floor(pr(i, 6.3) * 7)}px`,
    color:        CONFETTI_COLORS[Math.floor(pr(i, 8.1) * CONFETTI_COLORS.length)],
    opacity:      0.3 + pr(i, 4.1) * 0.35,
    borderRadius: pr(i, 9.3) > 0.5 ? '50%' : '2px',
  })), [])
    ...
    <>
      {pieces.map(p => (
        <div key={p.id} style={{
          ...
          animation: `wx-confetti ${p.dur} ${p.delay} linear infinite`,
        }} />
      ))}
      <div style={{
        position: 'absolute', top: '-20px', right: '-20px', width: '180px', height: '180px',
        background: 'radial-gradient(circle, rgba(245,158,11,0.18) 0%, rgba(251,113,133,0.1) 45%, transparent 70%)',
...
function PeacefulGlow() {
  return (
    <>
      <div style={{
        position: 'absolute', bottom: '-30px', right: '-30px', width: '210px', height: '200px',
        background: 'radial-gradient(ellipse, rgba(245,158,11,0.14) 0%, rgba(251,146,60,0.07) 45%, transparent 70%)',
        animation: 'wx-sun-glow 6s ease-in-out infinite',
        borderRadius: '50%',
      }} />
      <div style={{
        position: 'absolute', inset: 0,
        background: 'radial-gradient(ellipse at 80% 90%, rgba(245,158,11,0.05) 0%, transparent 55%)',
      }} />
    </>
  )
}
    ...
    <div className="p-3 rounded-xl bg-white/[0.04] border border-white/[0.06]">
      ...
      <button
        ...
        style={{
          ...
          background: copied ? 'rgba(52,211,153,0.12)' : `${ACCENT}18`,
        }}
      >
        {copied ? (
          <>
...
function CelebItem({ cel }) {
  ...
  const [msgs,       setMsgs]       = useState(null)   // null = not yet loaded
  ...
  const gifUrl = `https://giphy.com/search/${encodeURIComponent(
              ...
              <button
                onClick={() => {
                  if (aiUsed) {
                    ...
                    setMsgIndex(i => (i + 1) % displayMsgs.length)
                  }
                }}
                ...
                title={aiUsed ? 'Regenerate with AI' : 'Next message'}
              >
...
export default function CelebrationsCard({ data, loading, error, onRetry, delay = 0, syncedAt }) {
  ...
  // Keep activeTab in bounds if items change
      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', flex: 1 }}>
        ...
        {!loading && !error && items.length > 0 && (
          <div className="flex flex-col gap-3 flex-1">
            {/* Tab bar — only shown when more than one event */}
            {items.length > 1 && (
                ...
                {items.map((cel, i) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-semibold transition-all whitespace-nowrap flex-shrink-0"
                      style={{
